...
export default function UserRoutes(app) {
  ...
  const findEnrollmentsForUser = (req, res) => {
    const currentUser = req.session["currentUser"];
    const enrollments = enrollmentsDao.filterEnroll(currentUser._id);
    res.json(enrollments);
  };
  const enrollCourse = (req, res) => {
    const currentUser = req.session["currentUser"];
    const { courseId } = req.params;
    const enrollment = enrollmentsDao.enroll(currentUser._id, courseId);
    res.json(enrollment);
  };
  const unenrollCourse = (req, res) => {
    const currentUser = req.session["currentUser"];
    const { courseId } = req.params;
    enrollmentsDao.unenroll(currentUser._id, courseId);
    res.sendStatus(200);
  };
